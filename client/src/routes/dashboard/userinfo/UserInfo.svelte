...
<div class="rounded-md shadow-md md:p-10">
	<div class="w-full flex flex-row-reverse">
		<button class="btn" on:click={getUserInfo}>Refresh</button>
	</div>
	<div class="flex md:flex-row justify-between items-center mx-auto">
		<div class="p-4 mx-auto flex justify-between">
			<div class="flex justify-between mb-4 flex-col md:flex-row">
				<div class="md:w-1/4 rounded-full overflow-hidden">
				...
				{#if user.bankDetails}
					<div class="m-5">
						...
						<div>
							<span class="mr-5 font-semibold w-1/2">Account Number</span><span class=" w-1/2"
								>{user.bankDetails.bankAccountNumber}</span
							>
						</div>
						<div>
							<span class="mr-5 font-semibold w-1/2">IFSC Code</span><span class=" w-1/2"
								>{user.bankDetails.ifscCode}</span
							>
						</div>
						<div>
							<span class="mr-5 font-semibold w-1/2">PAN Number</span><span class=" w-1/2"
								>{user.bankDetails.panCard}</span
							>
						</div>
					</div>
				{:else}
