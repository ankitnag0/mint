<script>
  ...
  const handleSubmit = async() => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3000/users', {name, email, phoneNumber, password});
    ...
    {:else}
      <form class="lg:w-1/2 mx-auto flex flex-col content-center" on:submit|preventDefault={handleSubmit}>
          ...
          <div class="form-control w-full max-w-xs mx-auto">
              ...
              <input type="text" placeholder="Name" id="name" bind:value={name} required class="input input-bordered w-full max-w-xs" />
          </div>
          <div class="form-control w-full max-w-xs mx-auto">
              <label class="label" for="email">
                <span class="label-text">Email</span>
              </label>
              <input type="email" placeholder="Email" id="email" bind:value={email} required class="input input-bordered w-full max-w-xs" autocomplete="username" />
          </div>
          <div class="form-control w-full max-w-xs mx-auto">
              <label class="label" for="phoneNumber">
                <span class="label-text">Phone Number</span>
              </label>
              <input type="number" placeholder="Phone" id="phoneNumber" bind:value={phoneNumber} required class="input input-bordered w-full max-w-xs" />
          </div>
          <div class="form-control w-full max-w-xs mx-auto">
              <label class="label" for="password">
                <span class="label-text">Password</span>
              </label>
              <input type="password" placeholder="Password" id="password" bind:value={password} required class="input input-bordered w-full max-w-xs" autocomplete="current-password" />
          </div>
